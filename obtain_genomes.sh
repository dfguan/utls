...
do 
	...
	then
		...
		wget -c -o $outd/$dirn/"$dirn".wget.log   -O $outd/$dirn/"$dirn"."$ga".fna.gz "$gaftppath"/"$subdir"_genomic.fna.gz 
		wget -c -o $outd/$dirn/"$dirn".wget.log   -O $outd/$dirn/"$dirn"."$ga".gtf.gz "$gaftppath"/"$subdir"_genomic.gtf.gz 
		[ -s $outd/$dirn/"$dirn"."$ga".gtf.gz ] || rm -f $outd/$dirn/"$dirn"/"$ga".gtf.gz 
		wget -c -o $outd/$dirn/"$dirn".wget.log   -O $outd/$dirn/"$dirn"."$ga".gff.gz "$gaftppath"/"$subdir"_genomic.gff.gz 
		[ -s $outd/$dirn/"$dirn"."$ga".gff.gz ] || rm -f $outd/$dirn/"$dirn"/"$ga".gff.gz 
	fi		
